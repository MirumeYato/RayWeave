...
def make_collision_model(
        mu_a, mu_s, g, dt: float, 
        n_steps: int, Quadrature: Angle, c:int, 
        device, vebrose=0) -> StrangEngine:
    """
    ...
    observers = [EnergyLogger(every=n_steps//10), PlotMollviewInPoint([c,c,c], Quadrature, every=n_steps//10), EnergyPlotter(n_steps, every=n_steps//100)]
...
def get_analytical_solution(
        ...
        Quadrature: Angle, 
        device) -> torch.Tensor:
    J = initial_field.max() # intensity in point like delta t source.
    ...
    exp_lm = torch.exp(lambda_l * np.complex128(dt / 2.0)) * (2 * torch.arange(0, Lmax) + 1) # {L+1}
    ...
    final_field = speed * J * torch.einsum('p,qp->q', shperical_harmonics_H[point_s0] * exp_lm, shperical_harmonics) # [Q]
    return final_field
